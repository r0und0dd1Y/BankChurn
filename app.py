...
st.markdown(
    "Цей веб-додаток аналізує дані клієнтів та прогнозує, чи залишить клієнт банк (Churn)."
)
...
try:
    ...
    st.error(
        "Файл 'Churn_Modelling.csv' не знайдено. Будь ласка, додайте його в папку з проектом."
    )
    st.stop()
...
page = st.sidebar.radio(
    "Оберіть розділ:", ["📊 Аналіз даних", "⚙️ Метрики моделі", "🔮 Зробити прогноз"]
)
...
# --- Розділ 2: Метрики моделі ---
    ...
    st.markdown(
        "Модель натренована на 80% даних. Результати перевірки на тестовій вибірці:"
    )
    ...
    st.info(
        "💡 Зверніть увагу: Recall часто є найважливішою метрикою для бізнесу, адже нам важливо не пропустити жодного клієнта, який збирається піти."
    )
    ...
    with st.form("prediction_form"):
        ...
        with col1:
            credit_score = st.number_input(
                "Кредитний рейтинг", min_value=300, max_value=900, value=650
            )
            ...
            tenure = st.number_input(
                "Кількість років з банком", min_value=0, max_value=10, value=5
            )

        with col2:
            balance = st.number_input(
                "Баланс на рахунку ($)", min_value=0.0, value=50000.0
            )
            num_products = st.number_input(
                "Кількість продуктів банку", min_value=1, max_value=4, value=2
            )
            has_cr_card = st.selectbox("Чи має кредитну картку?", [1, 0])
            is_active = st.selectbox("Чи є активним клієнтом?", [1, 0])
            estimated_salary = st.number_input(
                "Очікувана зарплата ($)", min_value=0.0, value=60000.0
            )
